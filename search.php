<?php
require "htdoc.php"; // your DB connection

$search = isset($_GET['q']) ? trim($_GET['q']) : "";

if ($search === "") {
    $stmt = $conn->prepare(query: "SELECT * FROM recipe_db ORDER BY id DESC");
} else {
    $like = "%" . $search . "%";
    $stmt = $conn->prepare( "
        SELECT * FROM recipe_db
        WHERE name LIKE ? 
        OR subheading LIKE ? 
        OR cuisine LIKE ?
        OR food LIKE ?
        ...
        ORDER BY name ASC
    ");
    $stmt->bind_param("sssssss", $like, $like, $like, $like, $like, $like, );
}
    ...
    <main class="center-content"> 
        <div class="all-recipes-wrapper">
            <section class="header">
                ...
                <div class="div-row">
                    <form action="search.php" method="GET" class="search-bar">
                        <input type="text" id="search" name="q" placeholder="Search recipes..." />
                    ...
                    <a href="filters.html" class="icon-btn" id="filter-btn">
                ...
                <div class="all-recipes-section">
                    <?php
                        if ($result->num_rows === 0) {
                            echo "<p> No results found <br> Try searching something else up!</p>";
                        } else if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                $image_array = explode("*", $row["images"]);
                                $small_image_array = explode("*", $row["mobile_images"]);
                                $first_image = $image_array[0];
                                $small_first_image = $small_image_array[0];
                        
                                echo '
                                 <a href="recipe.php?name=' . urlencode(string: $row["name"]) . '" class="recipe-card-link">
                                     <div class="recipe-card">
                                        <div class="image-and-recipe-title">
                                            <picture>
                                                <source srcset="' . $small_first_image . '" media="(max-width: 600px)">
                                                <img src="' . $first_image . '" alt="' . htmlspecialchars(string: $row["name"]) . '">
                                            </picture>
                        
                                            <div class="recipe-card-title">
                                              <p class="recipe-name">' . htmlspecialchars(string: $row["name"]) . '</p>
                                              <p class="recipe-second-line">' . htmlspecialchars(string: $row["subheading"]) . '</p>
                                          </div>
                                       </div>
                        
                                       <div class="tags-line">
                                             <div class="recipe-tag"><p>' . htmlspecialchars(string: $row["cuisine"]) . '</p></div>
                                          <div class="recipe-tag"><p>' . htmlspecialchars(string: $row["food"]) . '</p></div>
                                     </div>
                                     </div>
                                 </a>' ;
                            }
            ...
            <!-- <div class="no-results-div">
                ...
                <p>Try again by searching something else!</p>
            </div> -->
        </div>
    </main>
    <footer>
